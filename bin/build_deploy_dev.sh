...
if [[ $BUILD_STATUS == 0 ]]; then
  ...
  sam deploy --stack-name=thepup-info-dev --s3-bucket=aws-sam-cli-managed-default-samclisourcebucket-1h3galvgqsgw0 --s3-prefix=thepup-info-dev --region=us-west-2 --capabilities=CAPABILITY_IAM --parameter-overrides="Env=\"dev\" XRayTracing=\"PassThrough\" ThePupsInfoSnippetsBucketName=\"the-pups-snippets-2020\" ThePupsOutputDataBucketName=\"the-pups-output-2020\" ThePupsUploadsBucketName=\"the-pups-uploads-2020\" ThePupsDogHistoryTableName=\"the-pups-campus-dog-history\" ThePupsDogSummaryTableName=\"the-pups-campus-dog-summary\" ThePupsCampusHistoryUpdatedSnsName=\"the-pups-campus-history-updated\""
else
  echo Build failed
fi
popd || exit
